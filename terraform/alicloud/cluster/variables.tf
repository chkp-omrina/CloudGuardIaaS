...
variable "allocate_and_associate_eip" {
  type = bool
  description = "If set to TRUE, an elastic IP will be allocated and associated with each cluster member, in addition to the cluster Elastic IPs"
  default = true
}
...
variable "ram_role_name" {
  type = string
  description = "A predefined RAM role name to attach to the cluster instances"
  default = ""
}
